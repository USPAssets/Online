// Script che controlla il testo "X was added to Y".
// Modificato per trattare DOLLARI OSCURI come special case
// per poter usare il plurale.
function scr_itemget_anytype_text()
{
    ...
    if (_itemtype == "money")
    {
        ...
        // Special case DOLLARI OSCURI per il plurale
        // Non uso stringsetsubloc per evitare di dover aggiungere una nuova chiave al json
        itemgetstring = stringsetsub("* (\\cY~1\\cW sono stati aggiunti a \\cY~2\\cW.)", itemname, itemtypename);
    } else if (noroom == 0)
